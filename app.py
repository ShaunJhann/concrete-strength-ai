...
if page == "Home":
    ...
    if st.button("Predict Strength"):
        try:
            ...
            st.success(
                f"Estimated Concrete Compressive Strength: "
                f"{prediction:.2f} MPa"
            )
            ...
            st.write(
                f"""
                The predicted concrete compressive strength is
                {prediction:.2f} MPa.

                This value represents the estimated ability of the
                concrete mixture to withstand compressive loading
                based on the input materials, curing age, and the
                trained Random Forest Regression model.

                The result is an AI-generated estimate and should
                be validated through laboratory testing before
                actual engineering use.
                """
            )

            st.caption(
                "This application is intended for educational and "
                "preliminary estimation purposes only."
            )

        except FileNotFoundError:
            st.error(
                "Model file not found. Put the .pkl file in the "
    ...
    st.write(
        "The project uses the UCI Concrete Compressive Strength "
        "dataset with eight input features and compressive "
    ...
    st.write(
        "A Random Forest Regressor is used to predict concrete "
    ...
    st.markdown(
        "[UCI Dataset](https://archive.ics.uci.edu/dataset/165/"
    ...
    st.write(
        "This project is relevant to Civil Engineering because "
        "concrete compressive strength is an important property "
        "in construction and structural engineering."
    )

    st.warning(
        "The application is for educational and preliminary "
        "estimation only. Actual concrete decisions require "
        "appropriate testing and professional judgment."
    )
